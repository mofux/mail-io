...
var mailer = {
	...
	createServer: function(config, cb/*session*/) {
		...
		var server = (config.secure ? tls : net).createServer(config.secure ? config.tls : {}, function(socket) {
			...
			socket.setTimeout(config.limits.idleTimeout);
			...
			socket.close = function(data) {
				...
				socket.write(data, function() {
					...
					// write has finsihed, destroy the socket
					socket.destroy();
		...
		server.handlers = mailer.getHandlers(config.handlers, config);
	 ...
	 */
	getHandlers: function(handlers, config) {
		...
		Object.keys(plugins).forEach(function(command) {

			plugins[command].forEach(function(handler, i) {
