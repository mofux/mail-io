	...
	var client = {
		...
		send: function(envelope, message, cb/*err*/) {
			...
			var onError = function(err) {
				if (returned) return;
				logger.log('warn', id, null, config.identity, 'error', null, { message: 'error while sending message for ' + [].concat(envelope.to).join(', ') + ' to ' + config.host  + (config.port !== 25 ? ':' + config.port : '') + ': ', data: err });
				returned = true;
				cb(err);
				connection.quit();
			}
