					...
					if (item instanceof Error) {
						if (item.stack) {
							item.stack.split('\n').forEach(function(line, i) {
								self.log(level, id, subId, event, subEvent, i === 0 ? 'up' : i === item.stack.length - 1 ? 'down' : 'line', { message: line }, dim);
							});
						} else {
							self.log(level, id, subId, event, subEvent, i === 0 ? 'up' : i === lines.length - 1 ? 'down' : 'line', colors.white(JSON.stringify(item)), dim);
						}
					} else {
						var lines = util.inspect(item).split('\r\n');
						lines.forEach(function(line, i) {
							self.log(level, id, subId, event, subEvent, i === 0 ? 'up' : i === lines.length - 1 ? 'down' : 'line', colors.white(line), dim);
						});
					}
