...
module.exports = function(opts) {
	...
	var config = extend(true, {
		...
		smtpPort: 25,
		// an optional smarthost that is used to deliver mail
		/*
		 smartHost: {
		 host: 'smarthost@mydomain.com',
		 port: 25,
		 auth: {
		 type: 'PLAIN',
		 username: 'myuser',
	...
	var relay = {
		...
		send: function(mail, cb/*err*/) {
			...
			async.each([].concat(mail.envelope.to || []), function(to, cb) {
				...
				dns.resolve(domain, 'MX', function(err, hosts) {
					...
					if (config.smartHost && config.smartHost.host) host = config.smartHost.host;
		...
		add: function(envelope, message, cb/*err*/) {
