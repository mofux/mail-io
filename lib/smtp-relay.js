...
module.exports = function(opts) {
	...
	var relay = {
		...
		reload: function(cb/*err*/) {
			...
			// try to create queue directory if it does not yet exist
			if (!fs.existsSync(config.queueDir)) try { fs.mkdirSync(config.queueDir) } catch(ex) { return cb('error creating queue directory "' + config.queueDir + '": ' + ex.message ) }
		...
		send: function(mail, cb/*err*/) {
			...
			async.each([].concat(mail.envelope.to || []), function(to, cb) {
				...
				dns.resolve(domain, 'MX', function(err, hosts) {
					...
					if (err || !_.isArray(hosts) || !hosts.length) hosts = [{ priority: 10, exchange: domain }];
					...
					// contact hosts in chain
					async.eachSeries(hosts, function(host, cb) {

						// get the ip address of the host
						dns.resolve(host, 'A', function(err, hosts) {

							// failed to resolve host, remember the error and continue with the next host
							if (err || !hosts || !hosts[0]) {

								errors.push({ permanent: false, msg: '[' + host + ']: failed to resolve A record for host "' + host + '": ' + (err || 'hostname not resolvable') });
								return cb();
							...
							client.send(mail.envelope, fs.createReadStream(mail.file), function(err) {
								...
								if (err) {
									...
									if (err.responseCode) {
										...
										// permanent failes have a response code >= 500
								...
								// the message has been successfully delivered, stop processing
								return cb(true);
						...
						// if we succeeded sending the message we can safely return here
						if (success) return cb();

						// failed to deliver the message to any of the mx targets
						var error = { permanent: true, msg: errors.length ? '' : 'failed to resolve any target hosts' };
