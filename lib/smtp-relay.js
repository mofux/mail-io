		...
		send: function(mail, cb/*err*/) {
			...
			async.each([].concat(mail.envelope.to || []), function(to, cb) {
				...
				dns.resolve(domain, 'MX', function(err, hosts) {
					...
					// by default, try to send to the domain if no mx record was found
					var host = domain;

					// try to resolve the mx record with the lowest priority
					if (!err && _.isArray(hosts) && hosts.length > 0) {

						host = hosts.sort(function(a, b) {
							if (!b) return -1;
							if (!a) return 1;
