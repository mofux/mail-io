...
module.exports = function(handlers, socket, secure, server, relay, config, cb) {
	...
	var session = {
		...
		reset: function() {
			...
			if (!session.accepted) session.accepted = {};
		...
		resetTransaction: function() {
			...
			delete session.accepted['rcpt'];
			delete session.accepted['mail'];
			delete session.accepted['data'];
			delete session.accepted['queue'];
		},
		...
		close: function(code, message) {
			...
			connection.socket.close(code && message ? code + ' ' + message + '\r\n' : null);
	...
	if (!session.client.address || !session.client.address.length) {
		session.log.warn('connect', 'core', 'client provides no ip address, disconnecting');
		return socket.close();
	}

	// resolve the remote hostname
	dns.reverse(session.client.address, function(err, hostnames) {

		// remember hostname
		session.client.hostname = !err && hostnames && hostnames.length ? hostnames[0] : '[' + session.client.address + ']';
